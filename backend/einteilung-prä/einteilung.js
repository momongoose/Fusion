...
async function insertRole(app){
    app.post("/einteilungPraeanalytik/role", async (req, res) => {
...
async function getRoles(app){
    app.get("/einteilungPraeanalytik/roles", async (req, res) => {
...
async function deleteRole(app){
    app.post("/einteilungPraeanalytik/remrole", async (req, res) => {
        var role = req.body.role
...
async function insertEinteilungF(app){
    app.post("/einteilungPraeanalytik/einF", async (req, res) => {
...
async function insertMitarbeiter(app){
    app.post("/einteilungPraeanalytik/addMA", async (req, res) => {
        var ma = req.body.ma
        ...
        if(esist == false) {
            ...
            await mySQl.addEinteilung("Mitarbeiter", ma, date.substring(0,10), type, "", "00:00:00", "00:00:00")
...
async function insertEinteilungS(app){
    app.post("/einteilungPraeanalytik/einS", async (req, res) => {
        var ein = req.body.ein
        var date = new Date(req.body.date)
        date.setDate(date.getDate() + 2);
        date = date.toISOString()
        ...
        if(esist == false) {
            for(var i = 0; i < ein.length; i++){
                ...
                for(var j = 0; j < list.length; j++){
                    ...
                    await mySQl.addEinteilung(role, list[j].name, date.substring(0,10), "s", list[j].state, list[j].start.slice(0,5).concat(':00'), list[j].ende.concat(':00'))
                }
            }
            ...
            for(var i = 0; i < ein.length; i++){
                ...
                for(var j = 0; j < list.length; j++){
                    ...
                    await mySQl.updateEinteilungComment(list[j].name, date.substring(0,10), list[j].state )
                    await mySQl.updateEinteilung(role, list[j].name, date.substring(0,10), "s", list[j].start.slice(0,5).concat(':00'), list[j].ende.concat(':00'))
                }
            }
...
async function insertEinteilungN(app){
    app.post("/einteilungPraeanalytik/einN", async (req, res) => {
        var ein = req.body.ein
        var date = new Date(req.body.date)
        date.setDate(date.getDate() + 2);
        date = date.toISOString()
...
async function remEin(app){
    app.post("/einteilungPraeanalytik/remEin", async (req, res) => {
...
async function updateBreak(app){
    app.post("/einteilungPraeanalytik/upBreak", async (req, res) => {
        var ma = req.body.ma
        var date = new Date(req.body.date)
        date.setDate(date.getDate() + 2)
        date = formatDate(date)
...
async function rename(app){
    app.post("/einteilungPraeanalytik/ren", async (req, res) => {
...
async function updatePos(app){
    app.post("/einteilungPraeanalytik/upPos", async (req, res) => {
...
async function getEinteilung(app) {
    app.post("/einteilungPraeanalytik/teilung", async (req, res) => {
        var date = new Date(req.body.date)
        date.setDate(date.getDate() + 2)
        date = formatDate(date)
        var esist = false
...
async function getStats(app){
    app.get("/einteilungPraeanalytik/getStats", async (req, res) => {
