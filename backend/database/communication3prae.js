...
class MySQLdb {
  ...
  async getSusers(date){
    var sql = "SELECT * FROM table WHERE datum = " + this.mysql.escape(date) + " ORDER BY start"
    let users = await new Promise((resolve, reject) => {
      this.db.query(sql, function (err, result) {
        if (err) {
          reject(err)
  ...
  async getNusers(date){
    var sql = "SELECT * FROM table WHERE start > '12:00:00' AND datum = " + this.mysql.escape(date)
    let users = await new Promise((resolve, reject) => {
      this.db.query(sql, function (err, result) {
        if (err) {
          reject(err)
