...
async function addBreakTime(app) {
  app.post("/einteilung/addBT", async (req, res) => {
...
async function deleteBreakTime(app) {
  app.post("/einteilung/delBT", async (req, res) => {
    var dep = req.body.dep;
    var val = req.body.value;
...
async function getSettings(app) {
  app.post("/einteilung/getSet", async (req, res) => {
    var dep = req.body.dep;
...
async function getMa(app) {
  app.get("/einteilung/MaSettings", async (req, res) => {
...
async function addGroup(app) {
  app.post("/einteilung/addGroup", async (req, res) => {
...
async function remGroup(app) {
  app.post("/einteilung/remGroup", async (req, res) => {
    var group = req.body.group;
...
async function getGroups(app) {
  app.get("/einteilung/getGroups", async (req, res) => {
...
async function addSkill(app) {
  app.post("/einteilung/addSkill", async (req, res) => {
...
async function remSkill(app) {
  app.post("/einteilung/remSkill", async (req, res) => {
    var skill = req.body.skill;
    var name = req.body.name;
    await mySQl.remSkill(name, skill) 
    res.status(200).send({ text: "skill removed" });
  });
}

async function getSkills(app) {
  app.get("/einteilung/getSkills", async (req, res) => {
...
async function updateRoleGroup(app) {
  app.post("/einteilung/upRoleGroup", async (req, res) => {
    var role = req.body.role;
    var group = req.body.group;
    await mySQl.updateRoleGroup(group, role) 
    res.status(200).send({ text: "Group to Role updated" });
  });
}

async function updateWish(app) {
  app.post("/einteilung/upWish", async (req, res) => {
