...
@Component({
  selector: 'app-admin-s-prae',
  templateUrl: './admin-s-prae.component.html',
  styleUrls: ['./admin-s-prae.component.css']
})
export class AdminSPraeComponent implements OnInit {
  ...
  update(){
    setInterval(()=>{
      ...
      this.breakTimes = this.app.settingsPräanalytik
      for(var s = 0; s < this.einteilung.length; s++){
        if(this.einteilung[s].list.length < 2){
          this.einteilung[s].list.push({name:"", state: "", start : "", ende : ""},{name:"", state: "", start : "", ende : ""})
        }
      }
    }, 1000)
  }
  ...
  formatGroupPeople() {
    ...
    var group = this.app.PraeGroups.text
    var skills = this.app.PraeSkills.text
  ...
  async submit(){
    this.waiting = true
    this.ein.inEinS(this.einteilung, this.app.date).subscribe((data)=>{
      return data
    })
  ...
  del:boolean = false
  delRole:any
  breakTimes:any = this.app.settingsPräanalytik
  ...
  Add(){
    if(this.name == ""){
      ...
      this.einteilung[0].list.push({name:this.name, state: "", start: "", ende: ""})
    }
  }
...
async addEvent(event: any) {
  ...
  for(var s = 0; s < this.app.einteilungS.length; s++){
    this.app.einteilungS[s].list = [{name:"", state: "", start : "", ende : ""},{name:"", state: "", start : "", ende : ""}]
  }
  ...
  for(var i = 0; i < this.app.spät.length; i++){
    if(this.app.spät[i].bemerkung.toLowerCase().includes('krank')){
      this.app.einteilungS[2].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start +"-", ende:this.app.spät[i].ende})
    } else if (this.app.spät[i].bemerkung.toLowerCase().includes('urlaub')){
      this.app.einteilungS[1].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start +"-", ende:this.app.spät[i].ende})
    } else {
      this.app.einteilungS[0].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start +"-", ende:this.app.spät[i].ende})
    }
  }
