...
@Component({
  selector: 'app-admin-s-analytik',
  templateUrl: './admin-s-analytik.component.html',
  styleUrls: ['./admin-s-analytik.component.css']
})
export class AdminSAnalytikComponent implements OnInit {
    ...
    for (var i = 0; i < roles.length; i++) {
      if(roles[i].type == "s"){
        for(var j = 0; j < this.einteilung.length; j++){
          if(roles[i].active == 0 && this.einteilung[j].id == roles[i].role + 's' && this.einteilung[j].list[0].name == "" ){
