...
@Component({
  selector: 'app-position-prae',
  templateUrl: './position-prae.component.html',
  styleUrls: ['./position-prae.component.css']
})
export class PositionPraeComponent implements OnInit {

  constructor(public app : AppComponent, private ein : EinteilungPraeService, private router : Router) {
  ...
  async addEvent(event: any) {
    ...
    for(var i = 0; i < this.app.spät.length; i++){
      if(this.app.spät[i].bemerkung.toLowerCase().includes('krank')){
        this.app.einteilungS[2].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start.slice(0, 5) +"-", ende:this.app.spät[i].ende.slice(0, 5)})
      } else if (this.app.spät[i].bemerkung.toLowerCase().includes('urlaub')){
        this.app.einteilungS[1].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start.slice(0, 5) +"-", ende:this.app.spät[i].ende.slice(0, 5)})
      } else {
        this.app.einteilungS[0].list.push({name:this.app.spät[i].name, state:this.app.spät[i].bemerkung, start:this.app.spät[i].start.slice(0, 5) +"-", ende:this.app.spät[i].ende.slice(0, 5)})
      }
    }
...
async getTableData(){
  ...
  for (let i = 0; i < Json.length; i++) {
      if(Json[i].type == "f"){
        ...
        this.app.einteilungS.push({ id: Json[i].role +"s", group: Json[i].group, list: [{name : "", state : "", start : "", ende : ""}], display: false, rename: false, color: Json[i].color})
