...
export class DisplayComponent implements OnInit {
  constructor(private router: Router, private app: AppComponent, private ein : EinteilungService) {
  ...
  update() {
    ...
    var now = moment()
    var startFS = moment('07:45','HH:mm');
    if (now.isBefore(startFS)) {
  ...
  async addEvent(event: any) {
    ...
    for (var n = 0; n < this.einteilungN.length; n++) {
      if(this.einteilungN[n].list.length == 0 ){
  ...
  rotate() {
    setInterval(() => {
      if(this.router.url !== "/display"){
        return
      }
      this.update();
      var now = moment()
      var startFS = moment('07:45','HH:mm');
      var startN = moment('20:45','HH:mm');
      if (now.isSameOrAfter(startFS) && now.isBefore(startN)) {
